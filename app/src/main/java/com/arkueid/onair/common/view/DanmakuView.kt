...
class DanmakuView(context: Context, attributeSet: AttributeSet?, defStyle: Int) :
    ...
    // 弹幕参数
    // 每秒移动的像素点
    private val baseSpeed: Float = 2.dp
    ...
    // 弹幕行间距
    private val danmakuVGap: Float = 10.dp

    // 同一行滚动弹幕之间间距
    private val danmakuHGap: Float = 15.dp
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        strokePaint.color = strokePaint.color.rgba
        ...
        for (danmakuItem in danmakus) {
            ...
            when (danmakuItem.style) {
                Danmaku.Style.ROLLING -> drawRollingDanmaku(canvas, danmakuItem)
                Danmaku.Style.TOP -> drawTopDanmaku(canvas, danmakuItem)
                Danmaku.Style.BOTTOM -> drawBottomDanmaku(canvas, danmakuItem)
            }
        }
    }
    ...
    private val Int.rgba: Int get() = this.and(0x00ffffff).or(danmakuAlpha shl 24)
    ...
    private fun DisplayDanmaku.draw(canvas: Canvas) {
        ...
        // 内容
        textPaint.color = this.color.rgba
